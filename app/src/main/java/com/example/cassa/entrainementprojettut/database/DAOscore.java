...
public class DAOscore extends DAOconnection {
    ...
    public void updateScore(Score score){
        int gameId=getGameIdFromGameName(score.getGameName());
    ...
    public int getGameIdFromGameName(String gameName){
        open();
        int gameId=0;
        String query="select idGame "+
                " from "+Constants.FIRST_TABLE +
                " WHERE "+Constants.COL_GAME_NAME_TABLE_1 +" =? ";
        Log.d("score",query);
        Cursor c=sqLiteDatabase.rawQuery(query,new String[]{gameName});
        while (c.moveToNext()){
            gameId=c.getInt(0);
